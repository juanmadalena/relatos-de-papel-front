...
export const CartDrawer = ({ isOpen, setIsOpen }: CartDrawerProps) => {
    ...
    return (
        <>
            <div onClick={() => setIsOpen(false)} className="fixed md:block bg-neutral-800 bg-opacity-40 top-0 h-screen left-0 md:w-[100dvw]" style={{display: isOpen ? 'block' : 'none'}}></div>
            <div className="w-screen md:w-[35dvw] h-screen bg-[#dedede] fixed top-0 right-0 shadow-lg transform transition-transform duration-300 ease-in-out flex justify-between flex-col" style={{ transform: isOpen ? 'translateX(0)' : 'translateX(100%)' }}>
